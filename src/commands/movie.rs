...
pub async fn submit(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    if args.is_empty() {
        ...
        let recent_periods = periods::get_most_recent_period(&db_pool.get().unwrap());
        
        if recent_periods.len() == 1 {
            let cur_period = recent_periods[0];
            ...
            if movie_subs.len() == 0 {
                let num_added = submissions::create_moviesub(&db_pool.get().unwrap(), &msg.author.id.to_string(), movie_submission, "test", cur_period.id);
                ...
                if let Some(reaction) = &update_sub_msg.await_reaction(&ctx).timeout(std::time::Duration::from_secs(10)).author_id(msg.author.id).await {
                    ...
                    let _ = match emoji.as_data().as_str() {
                        "✅" => {
                            ...
                            submissions::create_moviesub(&db_pool.get().unwrap(), &msg.author.id.to_string(), movie_submission, "test", cur_period.id);
            ...
            msg.channel_id.say(&ctx.http, "No current movie submission periods active.").await.unwrap();
        }
    }
    ...
    if recent_periods.len() == 1 {
        ...
        if movie_subs.len() >= 2 {
            ...
            if let Some(reaction) = &update_sub_msg.await_reaction(&ctx).timeout(std::time::Duration::from_secs(10)).author_id(msg.author.id).await {
                ...
                let _ = match emoji.as_data().as_str() {
                    "✅" => {
                        update_sub_msg.edit(ctx, |m| m.content("Starting roll")).await?;
                        update_sub_msg.delete_reactions(ctx).await?;
                    ...
                    "❎" => { 
                        update_sub_msg.edit(ctx, |m| m.content("Cancelling roll")).await?;
                        update_sub_msg.delete_reactions(ctx).await?;
            ...
            if start_roll == true {
                ...
                periods::end_period(&db_pool.get().unwrap(), cur_period);
...
#[command]
pub async fn startperiod(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    let db_pool = {
        let data_read = ctx.data.read().await;
        data_read.get::<DBConnectionContainer>().expect("Expected DBConnection in TypeMap.").clone()
    };
    ...
    if recent_periods.len() < 1 {
        periods::create_period(&db_pool.get().unwrap());
        msg.channel_id.say(&ctx.http, "Started new submission period!").await.unwrap();
    } else {
